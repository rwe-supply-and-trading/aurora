...
class FOAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        *,
        lv_repo_name: str = "kafou/aurora-era5-t1-latent-vectors",
        ...
        self.time_index = sorted(
            set(self.target_ds.time.to_numpy()) & set(self.lv_ds.time.to_numpy())
        )
    ...
    def df_to_targets(df: pd.DataFrame | pl.DataFrame):
        ...
        if isinstance(df, pd.DataFrame):
            ...
            data_vars = {
                x.name: xr.DataArray(x.to_numpy(), dims=("time",)) for x in df if x.name != "time"
            }
        ...
        arrays = [
            ds[x].to_numpy().astype(np.float32).reshape(len(ds[x]), 1) for x in sorted(ds.data_vars)
        ]
...
class ECMWFDataLoaderFOAM:
    ...
    Example use:
        ...
        # Get an icechunk session.
        client = arraylake.Client()
        repo = client.get_repo("kafou/aurora-ECMWF-samples")
        session = repo.readonly_session("main")
